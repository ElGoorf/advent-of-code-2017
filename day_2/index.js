/**
 * Created by Hussein on 17/12/2017.
 */
const input = [
...
// part 1 = sum of range of each row;
let total_part_1 = 0;

// part 2 = sum of the only whole divided number form each row
let total_part_2 = 0;
...
input.forEach(row => {
  ...
  row.forEach(entry1 => {
    ...
    row.forEach(entry2 => {
      if (entry1 !== entry2) {
        const remainder = entry1 % entry2;
        if (remainder === 0) {
          divided = entry1 / entry2;
          total_part_2 += divided;
        }
      }
    });
  });
...
console.log(`total for part 1:${total_part_1}`);
console.log(`total for part 2:${total_part_2}`);